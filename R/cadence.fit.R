cadence.fit <-
function(x, y, iter.max=500, n.hidden=2, hidden.fcn=tanh, distribution=NULL,
         sd.norm=Inf, init.range=c(-0.5, 0.5), method=c("optim", "Rprop"),
         n.trials=1, trace=0, delta.0=0.1, delta.min=1E-6, delta.max=50,
         epsilon=1e-8, step.tol=1e-6, f.target=-Inf, maxit.Nelder=200,
         max.exceptions=500)
{
    if (!is.matrix(x)) stop("\"x\" must be a matrix")
    if(!is.matrix(y)) stop("\"y\" must be a matrix")
    if(any(c(iter.max, n.hidden, n.trials) <= 0))
       stop("invalid \"iter.max\", \"n.hidden\", or \"n.trials\"")
    if(is.null(distribution)){
        distribution <- list(density.fcn=dnorm, parameters=c("mean", "sd"),
    ...
    for(nh in n.hidden){
        ...
        for(i in 1:n.trials){
            ...
            n.exceptions <- 0
            while(exception){
                weights <- cadence.initialize(x, nh, init.range, distribution)
                gradient <- fprime(weights, cadence.cost, epsilon, NULL,
                                   x=x, y=y, n.hidden=nh,
                                   hidden.fcn=hidden.fcn, distribution=
                                   distribution, sd.norm=sd.norm,
                                   valid=rep(TRUE, length(weights)))
                valid.cur <- gradient^2 > epsilon
                weights <- weights[valid.cur]
                if(trace>0) cat("nonzero weights =", valid.cur, "\n")
                if(method=="optim"){
                    weights <- try(suppressWarnings(optim(weights, cadence.cost,
                                       method="Nelder", control=list(
                                       maxit=maxit.Nelder, trace=trace,
                                       REPORT=1), x=x, y=y, n.hidden=nh,
                                       hidden.fcn=hidden.fcn, distribution=
                                       distribution, sd.norm=sd.norm,
                                       valid=valid.cur)$par),
                                    silent=trace==0)
                    output.cdn.cur <- try(suppressWarnings(optim(weights,
                                       cadence.cost, method="BFGS", control=
                                       list(maxit=iter.max, trace=trace,
                                       REPORT=1), x=x, y=y, n.hidden=nh,
                                       hidden.fcn=hidden.fcn, distribution=
                                       distribution, sd.norm=sd.norm,
                                       valid=valid.cur)),
                    ...
                    output.cdn.cur <- try(rprop(w=weights, f=cadence.cost,
                                       ...
                                       x=x, y=y, n.hidden=nh, hidden.fcn=
                                       hidden.fcn, distribution=distribution,
                                       sd.norm=sd.norm, valid=valid.cur),
                                       silent=trace==0)
                }
